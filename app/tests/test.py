...
class RateLimiter:
    ...
    def wait(self, func: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> Callable[P, R]:
        with self.lock:
            ...
            if wait_time > 0:
                logger.info(f"wait after call for: {wait_time} seconds")
                time.sleep(wait_time)
...
# === SETUP ===
global_tool_rate_limiter = RateLimiter(max_calls_per_minute=10)
time.sleep(5)
...
def test_rate_limit():
    ...
    print(f"Вызвано 10 раз за {total_time:.2f} секунд")

    # Проверка: задержка распределена равномерно
    for i in range(1, len(times)):
        delta = times[i] - times[i - 1]
        print(f"Интервал между вызовами {i}: {delta:.2f} секунд")
        assert delta >= 5.5, f"Интервал между вызовами слишком мал: {delta}"

    assert 54 <= total_time <= 66, (
        f"Общее время должно быть около 60 секунд, а не {total_time:.2f}"
    )
